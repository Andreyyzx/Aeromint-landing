...
(function () {
	...
	var	userAgent = navigator.userAgent.toLowerCase(),
		...
		plugins = {
			...
			firstLetterAbsolute:     document.querySelectorAll('.title-classic-first-letter'),
		};
	...
	$window.on('load', function () {
		...
		if (plugins.isotope.length) {
			for (var i = 0; i < plugins.isotope.length; i++) {
				...
				wrap.querySelector('.add-isotope-item').addEventListener('click', function() {
					let hiddenItems = wrap.querySelectorAll('.isotope-item-hidden');
					for( let i = 0; i < hiddenItems.length; i++ ) {
						hiddenItems[i].classList.remove('isotope-item-hidden')
					}
	...
	$(function () {
		...
		function updSwiperNumericPagination() {
			this.el.querySelector('.swiper-counter')
					.innerHTML = '<span class="count">' + ((this.realIndex + 1) < 10 ? '0' + (this.realIndex + 1) : (this.realIndex + 1)) + '</span>/<span class="total">' + (this.el.slidesQuantity < 10 ? '0' + this.el.slidesQuantity : this.el.slidesQuantity) + '</span>';
		}
		...
		/**
		 * @desc Set offset for firs letter
		 */
		function setOffsetFirstLetter(width, element) {
		...
		if (plugins.swiper.length) {
			for (var i = 0; i < plugins.swiper.length; i++) {
				...
				options.on = {
					init: function () {
						this.el.dispatchEvent( new Event( 'swiper:init' ) );
		...
		if (plugins.firstLetterAbsolute.length) {
			for (let i = 0; i < plugins.firstLetterAbsolute.length; i++) {
				let
						letter = plugins.firstLetterAbsolute[i],
						letterWidth = letter.clientWidth,
						letterParent = letter.parentElement;

				setOffsetFirstLetter(letterWidth, letterParent);
